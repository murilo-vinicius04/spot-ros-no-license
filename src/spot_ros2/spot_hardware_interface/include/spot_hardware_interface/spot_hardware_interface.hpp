...
class StateStreamingHandler {
 public:
  ...
  void get_states(JointStates& joint_states, ImuStates& imu_states, std::vector<int>& foot_states,
                  ...
                  std::vector<double>& vision_rot, std::vector<double>& odom_lin_vel,
                  std::vector<double>& odom_ang_vel);
  /**
 ...
 private:
  ...
  std::vector<double> lin_vel_body_odom_;      // (x, y, z) in m/s
  std::vector<double> ang_vel_body_odom_;      // (x, y, z) in rad/s
...
class SpotHardware : public hardware_interface::SystemInterface {
 ...
 private:
  ...
  static constexpr size_t n_odom_body_twist_sensor_interfaces_ = 6;
  ...
  // ADDED: Basic state client for no-license operation
  ::bosdyn::client::RobotStateClient* basic_state_client_{nullptr};
  std::shared_ptr<StateStreamingHandler> state_streaming_handler_{nullptr};
  ...
  std::vector<double> odom_lin_vel_;
  std::vector<double> odom_ang_vel_;

  // Thread for reading the state of the robot.
  std::jthread state_thread_;
  // NOTE: state_streaming_handler_ is now declared as shared_ptr above
  ...
  /**
   * @brief Send arm command using basic API (no streaming license required)
   * @param joint_commands JointCommands containing arm joint positions
   */
  void send_arm_command_basic(const JointCommands& joint_commands);
  
  /**
   * @brief Send body pose command using mobility API (no streaming license required)
   * @param joint_commands JointCommands containing body joint positions
   */
  ...
  std::vector<double> hw_odom_body_twist_sensor_states_;  // Holds the odom to body twist (lin & ang vel)
  std::vector<double> hw_vision_body_sensor_states_;      // Holds the vision to body transforms

  std::shared_ptr<rclcpp::Logger> logger_;

  bool use_streaming_license_ = false;

  // Parameters
  // These are read from the YAML file passed to the controller manager
};

// ADDED: Function declaration for basic state polling loop (no streaming license required)
